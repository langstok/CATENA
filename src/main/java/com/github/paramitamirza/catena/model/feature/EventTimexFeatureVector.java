...
import com.github.paramitamirza.catena.parser.entities.Sentence;
...
public class EventTimexFeatureVector extends PairFeatureVector{
	...
	public ArrayList<String> getEntityAttributes() {
		...
		entityAttrs.add(getEntityAttribute(e2, FeatureEnum.FeatureName.timexType) + "|" +
		...
		entityAttrs.add(getEntityAttribute(e2, FeatureEnum.FeatureName.dct));
		...
		if ((e2 instanceof Timex && ((Timex)e2).isDct()) || (e2 instanceof Timex && ((Timex)e2).isEmptyTag())) {
			...
			if (tidxStart > tidxStartSent) {
				...
				if (depPath.contains("TMP-PMOD") 
					...
					String beforeTmx = doc.getTokens().get(doc.getTokenArr().get(tidxStart-1)).getTokenAttribute(FeatureEnum.FeatureName.lemma);
				...
				if (eidx < entArr.size()-1 && doc.getEntities().get(entArr.get(eidx+1)) instanceof Timex) {
					...
					String beforeTmx1 = doc.getTokens().get(doc.getTokenArr().get(tidxStart-1)).getTokenAttribute(FeatureEnum.FeatureName.lemma);
					String beforeTmx2 = doc.getTokens().get(doc.getTokenArr().get(tmx2Idx-1)).getTokenAttribute(FeatureEnum.FeatureName.lemma);
					...
					String beforeTmx1 = doc.getTokens().get(doc.getTokenArr().get(tmx1Idx-1)).getTokenAttribute(FeatureEnum.FeatureName.lemma);
					String beforeTmx2 = doc.getTokens().get(doc.getTokenArr().get(tidxStart-1)).getTokenAttribute(FeatureEnum.FeatureName.lemma);
